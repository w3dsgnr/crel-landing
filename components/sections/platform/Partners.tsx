...
// v3: плитки на --bg-mist, секция на атмосферной плоскости --bg-bloom.
  ...
  return (
    <section className="bg-bloom">
      <div className="mx-auto max-w-[1200px] px-5 pb-28 md:px-12 md:pb-40">
        <p className="text-label text-pine-600">{partners.section.label}</p>
        ...
        <div className="mt-16 grid grid-cols-2 gap-4 md:grid-cols-4" data-reveal>
          {Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              className="flex h-20 items-center justify-center rounded-(--radius-m) bg-bg-mist text-label text-ink-soft"
            >
              [logo]
            </div>
          ))}
