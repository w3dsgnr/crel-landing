...
// Последовательность: c:rel → 1000мс → c:platform → 1000мс → c:services → … по кругу.
...
const WORDS = ["rel", "platform", "services"] as const;
const HOLD_MS = 1000; // выдержка слова перед стиранием
  ...
  useEffect(() => {
    ...
    const step = (index: number) => {
      schedule(() => {
        ...
        typewriterRef.current.retype(next);
